...
public class CanIWin {
	...
	public boolean canIWin(int maxChoosableInteger, int desiredTotal) {
        ...
        Map<Integer, Boolean> picks = new HashMap<>();

        return canWin(picks, 0, 0, maxChoosableInteger, desiredTotal);
    }

    private boolean canWin(Map<Integer, Boolean> picks, int used, int sum, int max, int desire) {
    	if (sum >= desire) {
    		return false;
    	}

    	if (picks.containsKey(used)) return picks.get(used);
    	for (int i = 1; i <= max; i++) {
    		if (((used >> i) & 1) == 0) {
    			boolean win = canWin(picks, used | (1 << i), sum + i, max, desire);
    			if (win) {
    				picks.put(used, false);
    			} else {
    				picks.put(used, true);
    				return true;
    			}
